...
const QuantityStat = () => {
  ...
  const quantityOverall = useSelector((state) => state.outlet.quantityOverall);

  let data = [];

  for (let key in quantityOverall) {
    if (key !== '_id' && key[key.length - 1] !== 't') {
      data.push({
        title: key,
        value: quantityOverall[key],
        last: 'Kg.',
      })
    }
  }
